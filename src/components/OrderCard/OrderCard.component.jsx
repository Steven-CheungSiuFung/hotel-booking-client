...
const OrderCard = ({order}) => {
    let locationAndCoordinates;
    if (order.hotel) {
        locationAndCoordinates = JSON.parse(order.hotel.location);        
    }
    ...
    return (
        <>
            ...
            :
            <div className="cotainer-fluid order-card-container d-flex justify-content-between">
                <p className="h6 d-flex align-items-center"><small className="text-muted">Booking ID: {order._id}</small></p>
                <p className="d-flex align-items-center">Post Deleted</p>
            </div>
            }
        </>
    )
